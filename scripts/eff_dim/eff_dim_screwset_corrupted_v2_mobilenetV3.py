...
# Hessian computation settings (match reference logic)
K = 128
MAXITER = 160
z = 1e-4                 # smoothing constant (same as weight decay)
...
OUTPUT_DIR = "/root/arcade/final_scripts/final_results/screwset-results/corrupt"
...
def get_hessian_loader(corruption_name):
    subset_dir = f'/root/arcade/data/screwset_split/hessian_subset_corrupted/{corruption_name}'
    if not os.path.isdir(subset_dir):
        print(f"Subset not found: {subset_dir}, skipping.")
        return None
    classes = [d for d in sorted(os.listdir(subset_dir))
    ...
    loader = DataLoader(ds, batch_size=HESS_BSZ, shuffle=False,
...
def estimate_top_eigs_regularized(model, hessian_loader, device, k=K, maxiter=MAXITER, epsilon=REG_EPSILON, z=z):
    ...
    def hvp_prod(vecs):
        # vecs: [n, m] where m is the number of Lanczos vectors requested at once
        outs = []
...
# ---- FP32 Hessian for all corruptions ----
if RUN_FP32:
    for corr in CORRUPTIONS:
        ...
        np.save(os.path.join(OUTPUT_DIR, f"eigs_fp32_screwset_{corr}.npy"), eigs32)
        meta32 = {"model":"mobilenetv3_screwset_fp32","dataset":f"screwset_{corr}","K":K,"z":z,"neff":neff32}
        with open(os.path.join(OUTPUT_DIR, f"neff_meta_fp32_screwset_{corr}.json"),"w") as f:
            json.dump(meta32, f, indent=4)

# ---- FP8 quantization, calibration, and bias correction (once) ----
if RUN_FP8:
    ...
    # ---- FP8 Hessian for all corruptions ----
    for corr in CORRUPTIONS:
        ...
        np.save(os.path.join(OUTPUT_DIR, f"eigs_fp8_e4m3_screwset_{corr}.npy"), eigs8)
        meta8 = {"model":"mobilenetv3_screwset_fp8_e4m3","dataset":f"screwset_{corr}","K":K,"z":z,"neff":neff8}
        with open(os.path.join(OUTPUT_DIR, f"neff_meta_fp8_e4m3_screwset_{corr}.json"),"w") as f:
