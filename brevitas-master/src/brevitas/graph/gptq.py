...
# Put these at the top of your class (just once)
DEBUG_EVERY    = 8   # only print on calls 10,20,30,...
DEBUG_MAX_CALLS = 35  # after 50 prints, stop printing entirely
...
class GPTQ(GPxQ):
    ...
    def compute_iterative_covariance(self, module, input, current_layer):
        ...
        # do_log = (self._dbg_calls <= DEBUG_MAX_CALLS) and (self._dbg_calls % DEBUG_EVERY == 0)
        do_log = False

        # 1) Update reference to current layer
        current_layer.layer_names.add(self.name)
        inp = self.process_input(input)
        batch_size = inp.shape[0]

        # 2) Preprocess the input to compute the Hessian
        if isinstance(self.layer, nn.Linear):
            if inp.ndim > 2:
                inp = inp.reshape(-1, int(torch.prod(torch.tensor(inp.shape[2:]))))
            inp = inp.t()
            inp_processed = inp.unsqueeze(0)
            # if do_log:
            #     print(f"[DEBUG][{self.name}] Linear inp_processed shape: {inp_processed.shape}", flush=True)
            #     pass
            ...
            chunks = torch.chunk(inp, self.groups, dim=1)
            pieces = []
            for idx, c in enumerate(chunks):
                grp = unfold(c).transpose(1, 0).flatten(1)
                pieces.append(grp)
                if do_log:
                    continue
                    # print(f"[DEBUG][{self.name}] group {idx} inp_processed shape: {grp.shape}", flush=True)
            inp_processed = torch.stack(pieces)
            # if do_log:
            #     print(f"[DEBUG][{self.name}] Stacked inp_processed shape: {inp_processed.shape}", flush=True)

        else:
            # nothing to do for other layer types
            return

        # 3) Overall shape before Hessian
        # if do_log:
        #     print(f"[DEBUG][{self.name}] Input for Hessian shape: {inp_processed.shape}", flush=True)

        # 4) Stats *before* update
        if self.nsamples > 0 and do_log:
            diag0 = torch.diag(self.H[0])
            print(
                f"[DEBUG][{self.name}] Hessian diag BEFORE: "
                f"mean={diag0.mean():.6f}, std={diag0.std():.6f}, "
                f"min={diag0.min():.6f}, max={diag0.max():.6f}, "
                f"first5={diag0[:5].cpu().numpy()}",
                flush=True
            )

        # 5) Accumulate Hessian
        if self.nsamples > 0:
            self.H *= (self.nsamples / (self.nsamples + batch_size))
        self.nsamples += batch_size
        scaled = math.sqrt(2.0 / self.nsamples) * inp_processed.to(torch.float32)
        self.B.copy_(scaled.bmm(scaled.transpose(2, 1)))
        self.H += self.B

        # 6) Stats *after* update
        if do_log:
            diag1 = torch.diag(self.H[0])
            print(
                f"[DEBUG][{self.name}] Hessian diag AFTER: "
                f"mean={diag1.mean():.6f}, std={diag1.std():.6f}, "
                f"min={diag1.min():.6f}, max={diag1.max():.6f}, "
                f"first5={diag1[:5].cpu().numpy()} | samples={self.nsamples}",
                flush=True
            )
