...
parser.add_argument(
    '--validation-dir',
    required=False,
    help='Path to folder containing Imagenet validation folder (required if --validation-dirs is not provided)')
parser.add_argument(
    '--validation-dirs',
    nargs='+',
    default=None,
    help='Paths to one or more validation folders (clean and corruptions). If specified, --validation-dir is ignored.'
)
parser.add_argument(
    '--dataset',
    default='imagenet',
    choices=['imagenet', 'cifar10', 'screwset'],
...
parser.add_argument(
    '--export-dir', default='/root/arcade/quantized_screwset', type=str, help='Directory where to store the exported models')
...
parser.add_argument(
    ...
    type=str,
    required=True,
    help='Path to the FP32 ScrewSet model checkpoint to quantize')
parser.add_argument(
    '--dtype',
    default='float',
    choices=['float', 'bfloat16', 'float16'],
    help='Data type to use (float for FP32, bfloat16, or float16 for FP16)')
parser.add_argument(
    '--target-backend',
    default='fx',
...
parser.add_argument(
    ...
    default=None,
    type=str,
    help='Path to write final top-1 accuracy')

# New argument for exporting the quantized model and exiting
parser.add_argument(
    '--export-ptq-model',
    default=None,
    type=str,
    help='If set, save the calibrated quantized model to this path and exit before validation'
)
...
def main():
    ...
    # Require at least one of --validation-dir or --validation-dirs
    if args.validation_dirs is None and args.validation_dir is None:
        parser.error("At least one of --validation-dir or --validation-dirs must be specified.")

    # Support multiple validation directories
    val_dirs = args.validation_dirs if args.validation_dirs is not None else [args.validation_dir]
    val_dirs = [v for v in val_dirs if v]  # remove None/empty
    ...
    if args.dataset == 'cifar10':
        ...
        # Create CIFAR-10 datasets and dataloaders
        calibration_dataset = datasets.CIFAR10(
            root="./data", train=True, transform=calibration_transform, download=True
        )

        # Get the desired total number of calibration samples
        num_calib_samples = args.calibration_samples  # e.g., 1000

        # Extract targets from the CIFAR-10 training dataset
        targets = np.array(calibration_dataset.targets)

        # Get the unique classes and compute how many samples per class are needed
        unique_classes = np.unique(targets)
        samples_per_class = num_calib_samples // len(unique_classes)

        # Collect indices for each class in a balanced way
        balanced_indices = []
        for cls in unique_classes:
            cls_indices = np.where(targets == cls)[0]
            np.random.shuffle(cls_indices)
            selected_cls_indices = cls_indices[:samples_per_class]
            balanced_indices.extend(selected_cls_indices.tolist())

        calibration_subset = Subset(calibration_dataset, balanced_indices)
        ...
        calib_loader = tqdm(calib_loader, desc="Calibration", leave=True)

    elif args.dataset == 'screwset':
        # Normalize values for ScrewSet
        SCREWSET_MEAN = (0.7750, 0.7343, 0.6862)
        SCREWSET_STD  = (0.0802, 0.0838, 0.0871)
        calibration_transform = transforms.Compose([
            transforms.Resize((240, 320)),
            transforms.ToTensor(),
            transforms.Normalize(mean=SCREWSET_MEAN, std=SCREWSET_STD),
        ])
        test_transform = transforms.Compose([
            transforms.Resize((240, 320)),
            transforms.ToTensor(),
        ...
        # Load full ScrewSet training data for calibration
        calibration_dataset = ImageFolder(root=args.calibration_dir, transform=calibration_transform)
        num_calib_samples = args.calibration_samples
        targets = np.array(calibration_dataset.targets)
        unique_classes = np.unique(targets)
        samples_per_class = num_calib_samples // len(unique_classes)
        balanced_indices = []
        for cls in unique_classes:
            cls_indices = np.where(targets == cls)[0]
            np.random.shuffle(cls_indices)
            balanced_indices.extend(cls_indices[:samples_per_class].tolist())
        ...
        test_transform = test_transform  # already defined
    
    else:
        # Original ImageNet dataloader code:
        ...
        calib_loader = tqdm(calib_loader, desc="Calibration", leave=True)
        test_transform = None  # will use generate_dataloader for val dirs

    # Build DataLoaders for all validation directories
    val_loaders = {}
    if args.dataset == 'cifar10':
        from torchvision import datasets
        for vdir in val_dirs:
            # For CIFAR-10, vdir is ignored, always use test split
            test_dataset = datasets.CIFAR10(
                root="./data", train=False, transform=test_transform, download=True
            )
            loader = torch.utils.data.DataLoader(
                test_dataset, batch_size=args.batch_size_validation, shuffle=False, num_workers=args.workers
            )
            val_loaders[vdir] = tqdm(loader, desc=f"Validation {os.path.basename(vdir)}", leave=True)
    elif args.dataset == 'screwset':
        for vdir in val_dirs:
            ds = ImageFolder(root=vdir, transform=test_transform)
            loader = DataLoader(
                ds,
    ...
    model.eval()
    if args.dataset == 'screwset':
        # Adjust final layer to match ScrewSet number of classes
        first_val_dir = val_dirs[0]
        val_dataset = ImageFolder(root=first_val_dir, transform=test_transform)
        ...
        # Load the user-specified ScrewSet checkpoint
        ckpt_path = args.checkpoint_path
        if os.path.isfile(ckpt_path):
            print(f"Loading checkpoint for model {args.model_name}: {ckpt_path}")
            ckpt = torch.load(ckpt_path, map_location='cpu')
            # Debug: print shapes to diagnose conv1 mismatch
            try:
                print(f"Debug: Model conv1 weight shape = {model.features[0][0].weight.shape}")
            except Exception as e:
                print(f"Debug: could not access model.features[0][0].weight: {e}")
            try:
                print(f"Debug: Checkpoint conv1 weight shape = {ckpt['features.0.0.weight'].shape}")
            except Exception as e:
                print(f"Debug: could not access ckpt['features.0.0.weight']: {e}")
            model.load_state_dict(ckpt, strict=False)
            print(f"Loaded ScrewSet FP32 weights for model {args.model_name} from {ckpt_path}")
        else:
            raise FileNotFoundError(f"Checkpoint {ckpt_path} not found")
    elif args.dataset == 'cifar10':
        # Adapt final classifier to CIFAR-10
        num_classes = 10
        if hasattr(model, 'classifier'):
            in_feats = model.classifier[-1].in_features
            model.classifier[-1] = torch.nn.Linear(in_feats, num_classes)
            ...
            in_feats = model.fc.in_features
            model.fc = torch.nn.Linear(in_feats, num_classes)
        # Load the checkpoint provided by the user
        print(f"Loading CIFAR-10 checkpoint for model {args.model_name}: {args.checkpoint_path}")
        ckpt = torch.load(args.checkpoint_path, map_location='cpu')
        load_res = model.load_state_dict(ckpt, strict=False)
        if load_res.missing_keys or load_res.unexpected_keys:
            print(f"[!] Warning: missing_keys={load_res.missing_keys}, unexpected_keys={load_res.unexpected_keys}")
    ...
    print(
        f"Weight mantissa bit width: {args.weight_mantissa_bit_width} | "
        f"Weight exponent bit width: {args.weight_exponent_bit_width} | "
        f"Activation mantissa bit width: {args.act_mantissa_bit_width} | "
        f"Activation exponent bit width: {args.act_exponent_bit_width}"
    )
        # Debug: count activation-quant modules
    num_act_q = 0
    for m in quant_model.modules():
        if hasattr(m, 'act_quant') and m.act_quant is not None:
            num_act_q += 1
    print(f"Inserted {num_act_q} activation-quant modules")
    print(f"Quant format: {args.quant_format}")
    # Verify and report weight-quantizer bit-widths
    from collections import Counter
    weight_bw_hist = Counter()

    def extract_bit_width(wq_obj):
        """
        Recursively try to fetch a bit‑width from a weight quantizer proxy.
        Returns an int on success, or None on failure.
        """
        if wq_obj is None:
            return None
        # Direct attribute or callable
        bw_attr = getattr(wq_obj, "bit_width", None)
        if bw_attr is not None:
            bw_val = bw_attr() if callable(bw_attr) else bw_attr
            if torch.is_tensor(bw_val):
                bw_val = int(bw_val.item())
            return bw_val
        # Brevitas proxies usually carry a .tensor_quant field
        inner = getattr(wq_obj, "tensor_quant", None)
        if inner is not None and inner is not wq_obj:
            return extract_bit_width(inner)
        return None

    for mod in quant_model.modules():
        bw = extract_bit_width(getattr(mod, "weight_quant", None))
        if bw is not None:
            weight_bw_hist[bw] += 1
    print("Weight bit-width histogram:", dict(weight_bw_hist))
    # Report weight-quantizer class names for additional verification
    from collections import Counter
    wq_class_hist = Counter(
        type(mod.weight_quant).__name__
        for mod in quant_model.modules()
        if getattr(mod, 'weight_quant', None) is not None
    )
    print("Weight quantizer classes:", dict(wq_class_hist))
    # Report the *underlying* weight quantiser classes (inside the proxy)
    true_wq_hist = Counter(
        type(mod.weight_quant.tensor_quant).__name__
        for mod in quant_model.modules()
        if getattr(mod, "weight_quant", None) is not None
           and getattr(mod.weight_quant, "tensor_quant", None) is not None
    )
    print("Underlying weight quantiser classes:", dict(true_wq_hist))
    # Ensure quantized model is on the correct device
    quant_model = quant_model.to(device)
    # Move internal scaling buffers to the correct device
    for module in quant_model.modules():
        # Weight quant scaling buffer
        if hasattr(module, 'weight_quant') and module.weight_quant is not None:
            wq = module.weight_quant
            if hasattr(wq, 'scaling_impl') and hasattr(wq.scaling_impl, 'buffer') and wq.scaling_impl.buffer is not None:
                wq.scaling_impl.buffer = wq.scaling_impl.buffer.to(device)
        # Activation quant scaling buffer
        if hasattr(module, 'act_quant') and module.act_quant is not None:
            aq = module.act_quant
            # Some activations use fused proxy
            if hasattr(aq, 'fused_activation_quant_proxy'):
                proxy = aq.fused_activation_quant_proxy
                if hasattr(proxy, 'tensor_quant'):
                    tq = proxy.tensor_quant
                    if hasattr(tq, 'scaling_impl') and hasattr(tq.scaling_impl, 'buffer') and tq.scaling_impl.buffer is not None:
                        tq.scaling_impl.buffer = tq.scaling_impl.buffer.to(device)
    ...
    if args.act_scale_computation_type == 'static':
        ...
        print("==> Activation calibration completed")
    # # Debug: inspect activation quantizer scales after calibration
    # for module in quant_model.modules():
    #     if hasattr(module, "act_quant") and callable(module.act_quant.scale):
    #         try:
    #             scale = module.act_quant.scale()
    #             # print(f"{module.__class__.__name__} cached scale: {scale}")
    #         except Exception as e:
    #             # print(f"{module.__class__.__name__} error getting scale: {e}")

    if args.gpfq:
        print("==> Performing GPFQ")
        gpfq_loader = tqdm(calib_loader, desc="GPFQ", leave=True)
        apply_gpfq(
            gpfq_loader,
            quant_model,
            p=args.gpfq_p,
        ...
        print("==> GPFQ completed")

    if args.gpfa2q:
        print("==> Performing GPFA2Q")
        gpfa2q_loader = tqdm(calib_loader, desc="GPFA2Q", leave=True)
        apply_gpfq(
            gpfa2q_loader,
            quant_model,
            p=args.gpfq_p,
            ...
            compression_rate=args.compression_rate)
        print("==> GPFA2Q completed")

    if args.gptq:
        print("==> Performing GPTQ")
        gptq_loader = tqdm(calib_loader, desc="GPTQ", leave=True)
        apply_gptq(gptq_loader, quant_model, act_order=args.gpxq_act_order)
        print("==> GPTQ completed")

    if args.learned_round:
        print("==> Performing Learned Round")
        learned_round_loader = tqdm(calib_loader, desc="Learned Round", leave=True)
        apply_learned_round_learning(
            quant_model,
            learned_round_loader,
            iters=args.learned_round_iters,
            optimizer_lr=args.learned_round_lr)
        print("==> Learned Round completed")

    if args.calibrate_bn:
        print("==> Performing BN Calibration")
        bncal_loader = tqdm(calib_loader, desc="BN Calibration", leave=True)
        calibrate_bn(bncal_loader, quant_model)
        print("==> BN Calibration completed")


    # If user requested to export the quantized model, save and exit
    if args.export_ptq_model:
        os.makedirs(os.path.dirname(args.export_ptq_model), exist_ok=True)
        torch.save(quant_model, args.export_ptq_model)
        print(f"Saved quantized model to {args.export_ptq_model}")
        return
    else:
        # After calibration (and any post-calibration processing)
        print("Saving quantized model...")
        torch.save(
            quant_model.state_dict(),
            os.path.join(args.export_dir, config + ".pth"))

        # Validate quantized model on each validation directory
        compiled_model = torch.compile(quant_model, fullgraph=True, disable=not args.compile)
        for vdir, loader in val_loaders.items():
            print(f"==> Validating quantized model on {vdir}")
            with torch.no_grad(), quant_inference_mode(quant_model):
                # Capture top1 accuracy from validate
                top1 = validate(loader, compiled_model, stable=dtype != torch.bfloat16)
            print(f"==> Validation completed for {vdir} | acc@1: {top1:.3f}")
        ...
        if args.export_onnx_qcdq or args.export_torch_qcdq:
            ...
            images = images[:1]  # optionally reduce the batch size
