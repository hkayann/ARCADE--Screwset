...
# Argument parsing
# ----------------------
parser = argparse.ArgumentParser(description="Train CIFAR-10 baselines with ShuffleNetV2")
...
# Paths
# ----------------------
# DATA_DIR                = Path('/root/arcade/data/cifar10_split')
...
RESULTS_DIR             = Path('/root/arcade/final_scripts/final_results/shufflenetv2/baselines')
...
def train_and_evaluate(model, variant_name):
    ...
    best_val_acc      = 0.0
    best_model_path = SHUFFLENET_MODEL_DIR / f"{variant_name}_cifar10_best.pth"
    no_improve_epochs = 0
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        if val_acc > best_val_acc:
            ...
            if no_improve_epochs >= args.patience:
                print(f"[{variant_name}] Early stopping after {args.patience} epochs with no improvement.")
                break
    ...
    corrupt_results = {}
    CIFAR_C_DIR     = Path('/root/arcade/data/CIFAR-10-C')
    corr_labels     = np.load(CIFAR_C_DIR / 'labels.npy')
    for fname in sorted(os.listdir(CIFAR_C_DIR)):
        if not fname.endswith('.npy') or fname == 'labels.npy':
            continue
        images = np.load(CIFAR_C_DIR / fname)
...
# Save Results
# ----------------------
output_file = RESULTS_DIR / 'shufflenetv2_cifar10_baselines.json'
