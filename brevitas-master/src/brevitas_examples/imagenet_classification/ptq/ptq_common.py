...
from brevitas.graph.gprq import gprq_mode # New import
from brevitas.graph.gprq import GPRQ # New import
...
def apply_gprq(
        ...
        create_weight_orig=False):
    """
    Perform robust Hessian-aware quantization using GPRQ.
    """
    model.eval()
    dtype = next(model.parameters()).dtype
    device = next(model.parameters()).device
    with torch.no_grad():
        with gprq_mode(model,
                       act_order=act_order,
                       use_quant_activations=use_quant_activations,
                       create_weight_orig=create_weight_orig) as gprq:
            gprq_model = gprq.model
            for _ in tqdm(range(gprq.num_layers)):
