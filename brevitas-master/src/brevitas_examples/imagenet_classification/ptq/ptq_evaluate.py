...
# Add stochastic rounding argument
add_bool_arg(
    parser,
    'stochastic-round',
    default=False,
    help='Enable stochastic rounding for weight quantization (default: disabled)'
)
...
def main():
    ...
    custom_model_path = '/root/arcade/final_scripts/final_models/mobv3_sigmoid.pth'
    ...
    # --- Stochastic rounding toggle ---
    if args.stochastic_round:
        print("Applying stochastic rounding to all FloatQuant modules...")
        stoch_round = StochasticRoundSte(deterministic_inference=False, seed=SEED, device=device)
        for module in quant_model.modules():
            if isinstance(module, FloatQuant):
                module.float_to_int_impl = stoch_round
