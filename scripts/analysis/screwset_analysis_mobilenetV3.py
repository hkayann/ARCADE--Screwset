#!/usr/bin/env python3
"""
ScrewSet Hessian/Quantization Analysis (Sigmoid variant + z-sweep)

- Loads FP32 / FP8 eigenvalues (top-k after threshold).
- Computes spectral metrics + dim_eff(z) curves and summaries.
- Loads quantization accuracies from the ScrewSet JSON.
- Builds Delta-metrics vs Delta-accuracy tables and correlation matrices.
- Saves CSV + JSON to /root/arcade/final_scripts/final_results/eff_dim_analysis/screwset
"""

import json
import numpy as np
import pandas as pd
from scipy.stats import spearmanr, pearsonr
from scipy.optimize import curve_fit
from pathlib import Path

# ---------------------- CONFIG ----------------------
class CONFIG:
    # Paths
    eig_root_clean = Path("/root/arcade/final_scripts/final_results/screwset-results/clean")
    eig_root_corr  = Path("/root/arcade/final_scripts/final_results/screwset-results/corrupt")
    quant_json     = Path("/root/arcade/final_scripts/final_results/quantization/screwset/quant_results_matrix_screwset.json")
    out_dir        = Path("/root/arcade/final_scripts/final_results/eff_dim_analysis/screwset")

    # Eigen processing
    eigen_threshold = 1.0
    topk            = 128

    # Effective-dim sweep
    z_grid   = np.logspace(-4, 0, 13)
    auc_mode = "logz"      # integrate in log10(z) space

    # Which variant to analyze
    replacement = "hardsigmoid_to_sigmoid"
    z_effdim    = 1.0

    # Quant configs to correlate with FP32
    fp8_targets = ["PTQ_FP8_E4M3", "GPTQ_FP8_E4M3"]

    # File patterns (ScrewSet)
    fp32_clean_name = "eigs_fp32_screwset.npy"
    fp8_clean_name  = "eigs_fp8_e4m3_screwset.npy"
    fp32_prefix     = "eigs_fp32_screwset_hessian_subset_"
    fp8_prefix      = "eigs_fp8_e4m3_screwset_hessian_subset_"

    # Saving
    save_csv     = True
    save_json    = True
    pool_targets = True


# ---------------------- UTILS ----------------------
def norm_name(name: str) -> str:
    # Remove leading 'hessian_subset_' if present
    return name.replace("hessian_subset_", "")

def make_json_safe(obj):
    if isinstance(obj, dict):
        return {k: make_json_safe(v) for k, v in obj.items()}
    if isinstance(obj, list):
        return [make_json_safe(v) for v in obj]
    if isinstance(obj, (np.integer,)):
        return int(obj)
    if isinstance(obj, (np.floating,)):
        return float(obj)
    if isinstance(obj, np.ndarray):
        return make_json_safe(obj.tolist())
    if isinstance(obj, Path):
        return str(obj)
    return obj


# ---------------------- METRICS ----------------------
def spectral_metrics(eigs: np.ndarray, z_single: float) -> dict:
    eigs = np.sort(eigs)[::-1]
    keys = ["trace","largest_eig","head10","tail","pr","spectral_entropy","kappa","neff","alpha"]
    if eigs.size == 0:
        return {k: np.nan for k in keys}

    trace   = eigs.sum()
    largest = eigs[0]
    head10  = eigs[:10].sum() / trace if trace > 0 else np.nan
    tail    = eigs[10:].sum() / trace if trace > 0 else np.nan
    pr      = (trace**2) / (np.square(eigs).sum()) if trace > 0 else np.nan
    p       = eigs / trace if trace > 0 else np.zeros_like(eigs)
    p_nonzero = p[p > 0]
    spec_ent = -np.sum(p_nonzero * np.log(p_nonzero)) if p_nonzero.size > 0 else np.nan
    denom   = eigs[-1] if eigs[-1] > 1e-12 else 1e-12
    kappa   = largest / denom
    neff    = np.sum(eigs / (eigs + z_single))
    alpha   = power_law_alpha(eigs)

    return dict(trace=trace, largest_eig=largest, head10=head10, tail=tail,
                pr=pr, spectral_entropy=spec_ent, kappa=kappa, neff=neff, alpha=alpha)


def power_law_alpha(eigs: np.ndarray) -> float:
    y = np.sort(eigs)[::-1]
    x = np.arange(1, len(y) + 1)
    mask = y > 0
    if mask.sum() < 3:
        return np.nan
    logx, logy = np.log(x[mask]), np.log(y[mask])
    def lin(xx, a, b): return a * xx + b
    try:
        a, _ = curve_fit(lin, logx, logy)[0]
        return -a
    except Exception:
        return np.nan


def dim_eff_curve(eigs: np.ndarray, z_grid: np.ndarray) -> np.ndarray:
    eigs = eigs.reshape(-1, 1)
    return np.sum(eigs / (eigs + z_grid.reshape(1, -1)), axis=0)


def auc_and_slope(z_grid, neff_vals, mode="logz"):
    if len(z_grid) < 2 or np.any(np.isnan(neff_vals)):
        return np.nan, np.nan
    x = np.log10(z_grid) if mode == "logz" else z_grid
    auc   = np.trapz(neff_vals, x)
    slope = (neff_vals[-1] - neff_vals[0]) / (x[-1] - x[0])
    return auc, slope


# ---------------------- IO HELPERS ----------------------
def load_eigs(path: Path, thr: float, topk: int) -> np.ndarray:
    arr = np.load(path)
    arr = arr[arr > thr]
    if arr.size == 0:
        return arr
    arr = np.sort(arr)[::-1]
    return arr[:topk]


def collect_eigen_pairs(cfg: CONFIG):
    pairs = {}
    # Clean (optional)
    if cfg.fp32_clean_name and cfg.fp8_clean_name:
        fp32_clean = cfg.eig_root_clean / cfg.fp32_clean_name
        fp8_clean  = cfg.eig_root_clean / cfg.fp8_clean_name
        if fp32_clean.exists() and fp8_clean.exists():
            pairs["clean"] = {"fp32_path": fp32_clean, "fp8_path": fp8_clean}

    # Corruptions
    for f in cfg.eig_root_corr.glob(f"{cfg.fp32_prefix}*.npy"):
        raw = f.name.replace(cfg.fp32_prefix, "").replace(".npy", "")
        key = norm_name(raw)
        fp8_path = cfg.eig_root_corr / f.name.replace(cfg.fp32_prefix, cfg.fp8_prefix)
        if fp8_path.exists():
            pairs[key] = {"fp32_path": f, "fp8_path": fp8_path}
    return pairs


def load_quant_results(json_path: Path, replacement: str):
    with open(json_path, "r") as f:
        data = json.load(f)
    fp32 = {}
    fp8s = {}
    for r in data:
        if r.get("replacement") != replacement:
            continue
        if r.get("quant_type") == "FP32":
            fp32["clean"] = r["clean_accuracy"]
            fp32.update(r["corrupt_accuracy_per_type"])
        else:
            cfg_name = r["quant_config"]["name"]
            fp8s.setdefault(cfg_name, {})
            fp8s[cfg_name]["clean"] = r["clean_accuracy"]
            fp8s[cfg_name].update(r["corrupt_accuracy_per_type"])
    return fp32, fp8s


# ---------------------- CORRELATION ----------------------
def correlate_columns(df: pd.DataFrame, target_col: str) -> pd.DataFrame:
    out = []
    for col in df.columns:
        if col == target_col:
            continue
        if not (col.startswith("Delta_") or col.startswith("AUC_") or col.startswith("Slope_")):
            continue
        x, y = df[col], df[target_col]
        mask = x.notna() & y.notna()
        if mask.sum() < 3:
            continue
        out.append({
            "metric": col,
            "Pearson": pearsonr(x[mask], y[mask])[0],
            "Spearman": spearmanr(x[mask], y[mask]).correlation
        })
    if not out:
        return pd.DataFrame(columns=["metric","Pearson","Spearman"]).set_index("metric")
    return pd.DataFrame(out).set_index("metric").sort_values("Spearman")


def pooled_correlations(df: pd.DataFrame, target_cols: list) -> pd.DataFrame:
    long = []
    for t in target_cols:
        if t in df.columns:
            sub = df[[t]].rename(columns={t: "DeltaAccuracy"})
            sub["target"] = t
            long.append(sub)
    if not long:
        return pd.DataFrame()
    long_df = pd.concat(long, axis=0)

    metrics = [c for c in df.columns
               if (c.startswith("Delta_") or c.startswith("AUC_") or c.startswith("Slope_"))
               and c not in target_cols]

    pooled = []
    for m in metrics:
        x = np.repeat(df[m].values, len(target_cols))
        y = long_df["DeltaAccuracy"].values
        mask = ~np.isnan(x) & ~np.isnan(y)
        if mask.sum() < 3:
            continue
        pooled.append({
            "metric": m,
            "Pearson": pearsonr(x[mask], y[mask])[0],
            "Spearman": spearmanr(x[mask], y[mask]).correlation
        })
    if not pooled:
        return pd.DataFrame()
    return pd.DataFrame(pooled).set_index("metric").sort_values("Spearman")


# ---------------------- MAIN ----------------------
def main():
    cfg = CONFIG()
    cfg.out_dir.mkdir(parents=True, exist_ok=True)

    pairs = collect_eigen_pairs(cfg)
    acc_fp32, acc_fp8_dict = load_quant_results(cfg.quant_json, cfg.replacement)

    rows, per_corr = [], {}

    for corr_key, paths in pairs.items():
        if not (paths["fp32_path"].exists() and paths["fp8_path"].exists()):
            continue

        eig32 = load_eigs(paths["fp32_path"], cfg.eigen_threshold, cfg.topk)
        eig8  = load_eigs(paths["fp8_path"],  cfg.eigen_threshold, cfg.topk)

        m32   = spectral_metrics(eig32, cfg.z_effdim)
        m8    = spectral_metrics(eig8,  cfg.z_effdim)
        delta = {f"Delta_{k}": m8[k] - m32[k] for k in m32}

        neff32_curve = dim_eff_curve(eig32, cfg.z_grid)
        neff8_curve  = dim_eff_curve(eig8,  cfg.z_grid)
        neff_delta   = neff8_curve - neff32_curve

        auc32, slope32 = auc_and_slope(cfg.z_grid, neff32_curve, cfg.auc_mode)
        auc8,  slope8  = auc_and_slope(cfg.z_grid, neff8_curve,  cfg.auc_mode)
        auc_delta      = auc8 - auc32
        slope_delta    = slope8 - slope32

        # accuracy lookup (normalize names)
        name_for_acc = norm_name(corr_key)
        acc32 = acc_fp32.get(name_for_acc, np.nan)
        fp8_accs, deltas_acc = {}, {}
        for target in cfg.fp8_targets:
            acc8 = acc_fp8_dict.get(target, {}).get(name_for_acc, np.nan)
            fp8_accs[target]    = acc8
            deltas_acc[target]  = acc8 - acc32

        per_corr[corr_key] = {
            "metrics_fp32": m32,
            "metrics_fp8":  m8,
            "metrics_delta": delta,
            "dim_eff_curve": {
                "z": cfg.z_grid.tolist(),
                "fp32": neff32_curve.tolist(),
                "fp8":  neff8_curve.tolist(),
                "delta": neff_delta.tolist(),
                "auc_fp32": auc32,
                "auc_fp8":  auc8,
                "auc_delta": auc_delta,
                "slope_fp32": slope32,
                "slope_fp8":  slope8,
                "slope_delta": slope_delta
            },
            "acc_fp32": acc32,
            "acc_fp8":  fp8_accs,
            "DeltaAcc_fp8": deltas_acc
        }

        base = {
            "corruption": corr_key,
            "AUC_Delta_neff": auc_delta,
            "Slope_Delta_neff": slope_delta
        }
        base.update({f"DeltaAccuracy_{t}": d for t, d in deltas_acc.items()})
        base.update(delta)
        rows.append(base)

    df = pd.DataFrame(rows).set_index("corruption").sort_index()

    corr_tables = {}
    for target in cfg.fp8_targets:
        dacc_col = f"DeltaAccuracy_{target}"
        if dacc_col in df.columns:
            corr_tables[target] = correlate_columns(df, dacc_col)

    pooled_table = pd.DataFrame()
    if cfg.pool_targets and len(cfg.fp8_targets) > 1:
        df_renamed = df.rename(columns={f"DeltaAccuracy_{t}": t for t in cfg.fp8_targets})
        pooled_table = pooled_correlations(df_renamed, cfg.fp8_targets)

    # Save
    if cfg.save_csv:
        df.to_csv(cfg.out_dir / "screwset_delta_metrics_summary.csv")
        for tgt, tab in corr_tables.items():
            tab.round(4).to_csv(cfg.out_dir / f"corr_table_{tgt}.csv")
        if not pooled_table.empty:
            pooled_table.round(4).to_csv(cfg.out_dir / "corr_table_pooled.csv")

    corr_tables_dict = {t: tab.round(6).to_dict(orient="index") for t, tab in corr_tables.items()}
    pooled_dict      = pooled_table.round(6).to_dict(orient="index") if not pooled_table.empty else {}

    final_json = {
        "config": {k: v for k, v in vars(cfg).items() if not k.startswith('_')},
        "per_corruption": per_corr,
        "delta_metrics_table_csv": str(cfg.out_dir / "screwset_delta_metrics_summary.csv"),
        "correlation_tables": corr_tables_dict,
        "pooled_correlation_table": pooled_dict
    }
    final_json = make_json_safe(final_json)

    if cfg.save_json:
        with open(cfg.out_dir / "screwset_hessian_quant_analysis.json", "w") as f:
            json.dump(final_json, f, indent=2)

    print("[DONE] JSON saved to:", cfg.out_dir / "screwset_hessian_quant_analysis.json")
    print(df.head())
    for tgt, tab in corr_tables.items():
        print(f"\nCorrelations for {tgt}:\n", tab.round(3))
    if not pooled_table.empty:
        print("\nPooled correlations (PTQ+GPTQ):\n", pooled_table.round(3))


if __name__ == "__main__":
    main()