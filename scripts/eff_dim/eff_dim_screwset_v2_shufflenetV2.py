...
# --- Load ScrewSet ShuffleNetV2 model (change to 40 classes) ---
...
hessian_ds = CustomImageFolder(HESSIAN_SUBSET_DIR, transform=transform)
if len(hessian_ds.classes) != NUM_CLASSES:
    raise RuntimeError(f"Expected {NUM_CLASSES} class folders, found {len(hessian_ds.classes)}")
print(f"Hessian subset classes: {hessian_ds.classes}")
hessian_loader = DataLoader(hessian_ds, batch_size=HESS_BSZ, shuffle=False, num_workers=4, pin_memory=True)
...
if RUN_FP32:
    ...
    np.save(os.path.join(OUTPUT_DIR, "eigs_fp32_shufflenetv2_screwset.npy"), eigs32)
    with open(os.path.join(OUTPUT_DIR, "neff_meta_fp32_shufflenetv2_screwset.json"), "w") as f:
        json.dump({
            "model":   "shufflenetv2_x1_0_screwset",
            "dataset": "screwset", "K": K, "z": z, "neff": neff32
        }, f, indent=4)
...
if RUN_FP8:
    ...
    np.save(os.path.join(OUTPUT_DIR, "eigs_fp8_e4m3_shufflenetv2_screwset.npy"), eigs8)
    with open(os.path.join(OUTPUT_DIR, "neff_meta_fp8_e4m3_shufflenetv2_screwset.json"), "w") as f:
        json.dump({
            "model":   "shufflenetv2_x1_0_fp8_e4m3_screwset",
            "dataset": "screwset", "K": K, "z": z, "neff": neff8
        }, f, indent=4)
