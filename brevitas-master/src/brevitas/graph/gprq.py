...
# --- BEGIN: Load pre-computed Hessian traces at module level ---
import json

# Path to pre-computed Hessian traces
TRACES_FILE = '/root/arcade/final_scripts/final_results/hessian_trace_merged_annotated.json'
with open(TRACES_FILE, 'r') as _f:
    _HESSIAN_TRACES = json.load(_f)['layers']
# --- END: Load pre-computed Hessian traces ---


class GPRQ(GPxQ):
    """
    Adapted for robust Hessian-aware quantization (GPRQ), based on GPTQ:
    ...
    def __init__(
        ...
        assert torch_version >= version.parse('1.10'), "GPRQ requires torch 1.10 or higher"
    ...
    def single_layer_update(self, percdamp=.01, c=1e4):
        assert not self.layer.weight_quant.requires_quant_input, "Error: GPRQ does not support weight quantizers that require quantized inputs."
        ...
        # --- Robust ridge injection (constant diagonal w/ fixed λ=0.3 and clamp) ---
        if (self.clean_trace is not None) and (self.corrupt_trace is not None):
            # blended trace magnitude (λ fixed at 0.3)
            blended = (0.7 * self.clean_trace) + (0.3 * abs(self.corrupt_trace))
            for g in range(self.groups):
                # compute base ridge per dimension
                ridge = 0.3 * blended / self.columns
                # maximum allowed ridge from Hessian diagonal scale
                max_ridge = 0.1 * torch.mean(torch.diag(self.H[g]))
                # clamp ridge to avoid overcorrection
                ridge = torch.clamp(ridge, max=max_ridge)
                self.H[g].diagonal().add_(ridge)
            print(f"Layer {self.name}: Added robust ridge of {ridge:.4e}")
        ...
        try:
            ...
            warnings.warn(
                ...
                f'GPRQ will not be applied. '
                f'Increasing the number of samples might fix this issue')
            return
        finally:
            del self.H, self.B

        # Use the first group’s permutation for shaping error_block
        perm_global = permutation_list[0]

        for i1 in range(0, self.columns, self.blocksize):
            i2 = min(i1 + self.blocksize, self.columns)
            count = i2 - i1
            error_block = torch.zeros_like(
                weight[:, :, perm_global[i1:i2]], dtype=torch.float32)  # [groups, OC/groups, i2-i1]
...
class gprq_mode(gpxq_mode):
    """
    Apply GPRQ algorithm (robust multi-distribution Hessian quantization).

    Args:
        model (Module): The model to quantize with GPRQ
        ...
        inplace (bool): Whether to apply GPRQ inplace or perform a deepcopy. Default: True
        create_weight_orig (bool): If True, store the original floating point weights before applying
            GPRQ. These weights will be used anytime quantization is disabled. Default: True
        use_quant_activations (bool): Whether to leave quantize activations enabled while performing
            GPRQ. Default: False
        num_blocks (int): The number of sub-blocks to use to speed-up GPRQ computation. Default: 100
        ...
        gprq_class (GPRQ): The uninitialized class to perform GPRQ. Default: `brevitas.graph.gprq.GPRQ`

    Example:
        >>> with torch.no_grad():
        >>>     with gprq_mode(model) as gprq:
        >>>         gprq_model = gprq.model
        >>>         for i in tqdm(range(gprq.num_layers)):
    ...
    def __init__(
        ...
        # How many subblock to use during GPRQ for each layer
        self.num_blocks = num_blocks
        self.gprq_class = gprq_class
        # attach precomputed Hessian traces by layer name
        self.hessian_traces = _HESSIAN_TRACES
    ...
    def initialize_module_optimizer(self, layer, name, len_parallel_layers, create_weight_orig):
        # create the GPRQ optimizer instance
        optimizer = self.gprq_class(
            layer=layer,
            name=name,
        ...
        # attach the precomputed Hessian trace values
        trace_info = self.hessian_traces.get(name, {})
        optimizer.clean_trace   = trace_info.get('clean_trace', None)
        optimizer.corrupt_trace = trace_info.get('corrupt_trace', None)
        optimizer.delta         = trace_info.get('delta', None)
        print(f"[GPRQ Trace] Layer '{name}': clean_trace={optimizer.clean_trace}, corrupt_trace={optimizer.corrupt_trace}, delta={optimizer.delta}")
        return optimizer