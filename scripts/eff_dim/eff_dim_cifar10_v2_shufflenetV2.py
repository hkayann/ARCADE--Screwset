...
# --- Brevitas patches to avoid inf/nan checks breaking quant PTQ/GPTQ ---
...
# Quantization imports (we only need them in the FP8 block, but import here)
...
# --- Load CIFAR-10 ShuffleNetV2 model ---
model = models.shufflenet_v2_x1_0(weights=None)
in_features = model.fc.in_features
model.fc = nn.Linear(in_features, 10)
checkpoint_path = "/root/arcade/final_scripts/final_models/shufflenet_models/shufflenet_v2_x1_0_cifar10_best.pth"
...
# --- Hessian‐subset loader ---
if not os.path.isdir(HESSIAN_SUBSET_DIR):
    raise FileNotFoundError(f"Hessian subset directory not found: {HESSIAN_SUBSET_DIR}")
class_folders = sorted(os.listdir(HESSIAN_SUBSET_DIR))
if len(class_folders) != 10:
    raise RuntimeError(f"Expected 10 class folders, found {len(class_folders)}")
print(f"Hessian subset classes: {class_folders}")
hessian_ds = datasets.ImageFolder(HESSIAN_SUBSET_DIR, transform=transform)
hessian_loader = DataLoader(hessian_ds, batch_size=HESS_BSZ, shuffle=False,
...
if RUN_FP32:
    ...
    np.save(os.path.join(OUTPUT_DIR, "eigs_fp32_shufflenetv2_cifar10.npy"), eigs32)
    with open(os.path.join(OUTPUT_DIR, "neff_meta_fp32_shufflenetv2_cifar10.json"), "w") as f:
        json.dump({
            "model":   "shufflenetv2_x1_0_cifar10",
            "dataset": "cifar10", "K": K, "z": z, "neff": neff32
        }, f, indent=4)
    print(f"Saved FP32 results → {OUTPUT_DIR}")
...
if RUN_FP8:
    ...
    np.save(os.path.join(OUTPUT_DIR, "eigs_fp8_e4m3_shufflenetv2_cifar10.npy"), eigs8)
    with open(os.path.join(OUTPUT_DIR, "neff_meta_fp8_e4m3_shufflenetv2_cifar10.json"), "w") as f:
        json.dump({
            "model":   "shufflenetv2_x1_0_fp8_e4m3_cifar10",
            "dataset": "cifar10", "K": K, "z": z, "neff": neff8
        }, f, indent=4)
    print(f"Saved FP8 results → {OUTPUT_DIR}")
